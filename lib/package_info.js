...
//            var filename = path.join(app.options.src, json[key].filename);
            ...
            var filename = json[key].filename;

            // parse comma separated filename to support multiple files 
            var filenames = [];
            try {
            ...
            var contents = [];
            filenames.forEach(function (tempFilename) {
                var filePath = path.join(dir, tempFilename);
                if ( ! fs.existsSync(filePath))
                    filePath = path.join('./', tempFilename);
                try {
                    app.log.debug('read header file: ' + tempFilename);
                    var content = fs.readFileSync(filePath, 'utf8');
                    contents.push(content);
                } catch (e) {
                    throw new Error('Can not read: ' + tempFilename + '.');
                }
            });
            ...
            result[key] = {
                ...
                content: app.markdownParser ? app.markdownParser.render(contentStr) : contentStr
            };
        }
    });
